...
from libs.evaluation_utils_module import  rename, task_job, load_from_grid_search, load_from_optuna
...
DECAYS = {
    ...
    "cartpole":{
        "diff":[{"d":0.5, "fw":0.7}, {"d":1.0, "fw":0.8}, {"d":2.0, "fw":0.9}, {"d":3.0, "fw":1},{"d":1000.0, "fw":1}],
        "lambda":[{"d":0.5, "fw":0.7}, {"d":1.0, "fw":0.8}, {"d":2.0, "fw":0.9}, {"d":3.0, "fw":1},{"d":1000.0, "fw":1}]
    }
}
...
def evaluation_of_setup(en, alg, container, experiment_name, out_path, seeds, **kwargs):
    ...
    for future in concurrent.futures.as_completed(stat_futures):
        ...
        if "fitness" not in stats:
            stats["fitness"] = {}
        if dec["d"] not in stats["fitness"]:
            stats["fitness"][dec["d"]] = {}
        if ng not in stats["fitness"][dec["d"]]:  
            stats["fitness"][dec["d"]][ng] = {}

        stats["fitness"][dec["d"]][ng][s] = result[0]
        pops[s]= result[1] 
    ...
    json_path = Path(os.path.join(dirpath, filename + ".json"))
